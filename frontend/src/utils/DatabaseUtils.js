...
export const toggleVideo = async (videoId, playlistId, userEmail) => {
    try {
        const response = await axios.put(`${process.env.REACT_APP_BASE_URL}/video/toggle`, {
            userEmail,
            videoId,
            playlistId,
        });
...
export const setPlaylistIndex = async (userEmail, playlistId, playlistIndex) => {
    ...
    try {
        const response = await axios.put(`${process.env.REACT_APP_BASE_URL}/playlists/setVideoIndex`, {
            userEmail,
            playlistId,
            playlistIndex,
        });
