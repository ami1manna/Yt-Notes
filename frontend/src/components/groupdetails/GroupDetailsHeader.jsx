...
const GroupDetailsHeader = () => {
  ...
  const loading = useSelector(groupDetailSelectors.isGroupDetailsFetching);
  const error = useSelector(groupDetailSelectors.getGroupDetailsFetchError);

  if (loading) {
    return (
      <Card>
        <Card.Content className="p-6 text-gray-700 dark:text-gray-200">
  ...
  if (error || !group) {
    return (
      <Card>
        <Card.Content className="p-6 text-red-700 dark:text-red-400">
