...
const SummaryList = ({ videoId }) => {
  ...
  const mathJaxConfig = {
    ...
    startup: {
      typeset: true // Changed to true for automatic typesetting
    },
    options: {
      enableMenu: false, // Disables the context menu for better UX
      processSectionDelay: 0 // Process sections immediately
    },
    svg: {
      fontCache: 'global' // Better font caching
    }
  };
  ...
  // Effect to trigger MathJax typesetting when content changes
  useEffect(() => {
    // Only trigger typeset if component has mounted and notes are loaded
    if (hasMounted.current && notes && !notesLoading) {
  ...
  }, [notes, notesLoading, expandedSections, expandedTopics]);

  // Improved formula rendering function
  const renderFormula = (formula) => {
    // Ensure formula is properly wrapped in MathJax delimiters if not already
    const processedFormula = formula.startsWith('$') ? formula : `$${formula}$`;
    
    return (
      <MathJax inline={!formula.includes('$$')}>
        {processedFormula}
      </MathJax>
    );
  };

  // Helper function to process text that might contain LaTeX
  const processTextWithMath = (text) => {
    // Check if the text already has math delimiters
