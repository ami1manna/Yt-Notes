...
import { resetCreateState } from "@/store/group/groupSlice";

import AsyncStateHandler from "@/components/common/AsyncStateHandler";

const GroupForm = ({ isOpen, onClose, onGroupCreated }) => {
  const dispatch = useDispatch();
  const loading = useSelector(groupsSelectors.isGroupCreating);
  const error = useSelector(groupsSelectors.getGroupCreateError);
  ...
  return (
    <Modal isOpen={isOpen}>
      <div className="relative flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full">
        ...
        <div className="p-6">
          <AsyncStateHandler isLoading={false} error={error}>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* Privacy Selection */}
              <div>
                <span className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Privacy</span>
                <div className="grid grid-cols-2 gap-4">
                  {/* Public */}
                  <label className={`flex items-start gap-3 p-3 rounded-md border cursor-pointer text-sm ${
                    privacy === "public"
                      ? "border-blue-500 bg-blue-50 dark:bg-gray-700"
                      : "border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900"
                  }`}>
                    <div className="mt-1"><Globe className="w-5 h-5" /></div>
                    <div className="flex-1">
                      <div className="font-medium">Public</div>
                      <div className="text-xs text-gray-600 dark:text-gray-300">Anyone can find and join</div>
                    </div>
                    <input
                      type="radio"
                      name="privacy"
                  ...
                  <label className={`flex items-start gap-3 p-3 rounded-md border cursor-pointer text-sm ${
                    ...
                    <div className="mt-1"><Lock className="w-5 h-5" /></div>
                    <div className="flex-1">
                      <div className="font-medium">Private</div>
                      <div className="text-xs text-gray-600 dark:text-gray-300">Invite only</div>
                    </div>
                    <input
                      type="radio"
                      name="privacy"
