...
const StaticModal = ({ isOpen, onAction, header, subHeader, isLoading }) => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto outline-none focus:outline-none">
      {/* Overlay */}
      <div className="fixed inset-0 bg-black opacity-50 dark:bg-gray-900 dark:opacity-70"></div>
      ...
      <div className="relative w-full max-w-md mx-auto my-6">
        <div className="relative flex flex-col w-full bg-white dark:bg-gray-800 border-0 rounded-lg shadow-xl outline-none focus:outline-none">
          ...
          <div className="flex items-start justify-between p-5 border-b border-gray-300 dark:border-gray-700 rounded-t">
            <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-200">{header}</h3>
          </div>

          {/* Modal Body */}
          <div className="relative flex-auto p-6">
            <p className="my-4 text-base leading-relaxed text-gray-600 dark:text-gray-300">{subHeader}</p>
          </div>
