...
const SideNav = ({ playListData, selectedVideoId, setSelectedVideoId, setVideoStatus }) => {
  const { user } = useContext(AuthContext);
  const [isOpen, setIsOpen] = useState(false);
  const navRef = useRef(null);

  // Convert videoId-based navigation to match the existing logic
  const selectedVideoIndex = playListData?.videoOrder
    ? playListData.videoOrder.indexOf(selectedVideoId)
    : -1;
  ...
  // Convert videos to an array if they're in an object format
  const videosArray = playListData?.videoOrder
    ? playListData.videoOrder.map(videoId => playListData.videos[videoId])
    : [];
