...
const SunEditorComponent = ({playlistId,videoId}) => {
  ...
  useEffect(()=>{
    ...
    const fetchNotes = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/video/notes/${user.email}/${playlistId}/${videoId}`);
        if(response.data[0]) 
  ...
  const saveContent = async () => {
    try {
       setIsLoading(true);
      const response = await axios.put(`${process.env.REACT_APP_BASE_URL}/video/notes`, {
